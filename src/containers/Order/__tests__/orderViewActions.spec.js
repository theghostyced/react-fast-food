...
describe('Login Act{ions', () => {
  ...
  it(`should return an action object once ${type.FETCH_ORDER_START} is fired`, () => {
    expect(orderActions.fetchStart()).toEqual({
      type: type.FETCH_ORDER_START,
    });
  });

  it(`should return an action object once ${type.FETCH_ORDER_FAILED} is fired`, () => {
    expect(orderActions.fetchFailed()).toEqual({
      type: type.FETCH_ORDER_FAILED,
    });
  });

  it(`should return an action object once ${type.FETCH_ORDER_SUCCESS} is fired`, () => {
  ...
  it('should call the login start dispatch function', async () => {
    ...
    expect(dispatchFn).toBeCalledWith({ type: type.FETCH_ORDER_START });
  });
  ...
  it('should call the login success dispatch function', async () => {
    ...
    expect(dispatchFn).toBeCalledWith({ type: type.FETCH_ORDER_SUCCESS, payload: mockResponse });
  });
